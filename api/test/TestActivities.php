...
function test_HamtaAllaAktiviteter(): string {
    ...
    try {
        ...
        if($svar->getStatus()===200) {
            $retur .="<p class='ok'>Hämta alla aktiviteter returnareade 200 som förväntat</p>";
            if(array_key_exists("aktivities", $svar->getContent())) {
                $retur .= "<p class='ok'>" . count($svar->getContent()['aktivities']) . "poster returnerades</p>";
            } else {
                $retur .= "<p class='error'>Arrayen med poster ('aktivities') saknas</p>";
            }
        } else {
            $retur .="<p class='error'>Hämta alla aktiviteter returnareade {$svar->getStatus()}, 200 förväntat</p>";
        }
    } catch (Exception $ex) {
...
function test_HamtaEnAktivitet(): string {
    ...
    try {
        ...
        $poster=$alla->getContent()['aktivities'];
        $maxId=0;
        ...
        if($svar->getStatus()===400) {
            $retur .="<p class='ok'>hämta post med id som inte finns (". $maxId .") returnerade 400, som förväntat</p>";
        } else {
